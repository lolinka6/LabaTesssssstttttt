...
for(int i = 0; i < 10; i ++) // создаем и размещаем 10 платформ
{
...
{
horizontalD = "stay"; // если отпустили кнопку - горизонтальное движение останавливается
}
//---------------------------------------------------------------------------


int timesChanged = 0;
void __fastcall TForm3::Timer2Timer(TObject *Sender) // таймер для опускания платформ
{
	 if(timesChanged > 25)
	 {
		 timesChanged = 0;
	 ...
	 {
		 int min = 0;
		 for(int i = 0; i < 10; i ++) if(platforms[i]->Top < min) min = platforms[i]->Top; // находим высоту максимально высокой платформы
